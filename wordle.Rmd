...
pick_solution <- function(list) {
      ...
      answers <- sample (possible_answers, 1, replace = FALSE)
      answer_split <- strsplit (answers,"") [[1]]
}
```
...
```
{r}
pick_solution2 <- function(list) {
      possible_answers <- filter (list, nchar (list) == 5)
      answers <- sample(possible_answers, length(5), replace= FALSE)
      answer_split <- strsplit (answers,"") [[1]]
}
```
...
```
{r}
solution <- pick_solution2(solution_list)
```
